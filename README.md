...
Run the SQL in `supabase-schema.sql` from the Supabase Dashboard:
...
If you already created the database earlier, run the schema again after pulling updates. It is written to be mostly idempotent with `if not exists` guards.
